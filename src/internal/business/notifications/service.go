...
func (s *NotificationService) Create(timestamp time.Time, weekdays internal.WeekdaysSum, userId int, stationCode string) (*Notification, error) {
	normalizedTime := time.Date(1970, 1, 1, timestamp.Hour(), timestamp.Minute(), timestamp.Second(), 0, time.Local)
	id, err := s.store.Create(normalizedTime, weekdays, userId, stationCode)
